...
" python3 mujo_10_ileed_bc_offline.py  --Expert_idx 0  --total_steps 3e6  --env 1  --noisy True --data_augm both   --rl_seed 2  --Pi_range_select 0 ",
...
def main(args):
    ...
    if clean_data:
        models_dir = root_folder + "/logs_ileed_bc_clean/"
    else:
        models_dir = root_folder + "/logs_ileed_bc_noisy/"
    ...
    if not clean_data:
        save_dir = models_dir+env_name+"_BC_"+RL_alg+"_NetAgentSeed_" + str(expert_idxs[0]) + "-" + str(expert_idxs[-1])+"_RLseed_"+str(seed_value)+"_noisy-un"+suffix+"/"

    else:
        save_dir = models_dir+env_name+"_BC_"+RL_alg+"_NetAgentSeed_" + str(expert_idxs[0]) + "-" + str(expert_idxs[-1])+"_RLseed_"+str(seed_value)+"-un"+suffix+"/"
